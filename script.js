...
function calculateNTime(realTime) {
    ...
    const adjustedDayLengthMs = currentN * 60 * 60 * 1000;
    const secondsIntoN = Math.floor((adjustedMilliseconds % adjustedDayLengthMs) / 1000);

    const nTimeRate = currentN / 24; 
    const totalSecondsIn24h = Math.floor(secondsIntoN / nTimeRate);
...
function renderClockMode() {
    document.getElementById('content-area').innerHTML = `
        ...
        <div class="control-panel">
            ...
            <div id="n-value-display" style="text-align: center; font-weight: 500;">N = ${currentN} ${currentLang === 'ja' ? '時間' : 'Hours'}</div>
        </div>
    `;
    setupNControl(); 
    updateClock();
